...
个数前缀和，对于指定区间的元素出现的个数，用集合的思想，即相减即可
...
class Solution {
    public int[] minDifference(int[] nums, int[][] queries) {
        //个数前缀和，对于指定区间的元素出现的个数，用集合的思想，即相减即可
        //记n、m、k为nums长度、queries长度、nums中元素的取值范围
        //则时间复杂度： o(n + mk)  空间复杂度：o(nk)
        int n = nums.length, m = queries.length;
        int ans[] = new int[m], count[][] = new int[n + 1][101];
        for(int i = 0; i < n; i++)
        {
            count[i + 1] = Arrays.copyOf(count[i], 101);
            count[i + 1][nums[i]]++;
        }
        for(int i = 0; i < m; i++)
        {
            int l = queries[i][0], r = queries[i][1], min = 100;
            for(int j = 1, pre = 0; j < 101; j++)
            {
                if(count[r + 1][j] - count[l][j] > 0)
                {
                    if(pre != 0) min = Math.min(min, j - pre);
                    pre = j;
                }
            }
            //min仍为100，则说明区间[l, r]中无不同的元素，则[l, r]子数组差绝对值的最小值为-1
            ans[i] = min == 100 ? -1 : min;
        }
        return ans;
    }
