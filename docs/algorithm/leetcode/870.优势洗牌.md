...
- 通过index数组将nums2从大到小排序，并记录nums2中每个数原来对应的下标。
- nums1从小到大排序，通过left和right两个指针确定nums1中[left，right]之间的数没有被使用过
- 对于nums2中最大的数max，如果nums1中有比max大的数直接选择，如果没有就选择nums1中最小的数。
- 将结果存在nums2中并返回以节省空间
